...
//
// Programado con launchd: scripts/launchd/com.somosmagma.diaria.plist (las instrucciones están ahí).
...
const texto = `${brief}\n\n\n# 🧾 El contador (Diego)\n\n${contador}\n\n---\nGenerado por scripts/diaria.mjs · ${hoy.toLocaleString('es-AR')}`
console.log(texto)
...
if (MAIL) {
  ...
  const subject = `☀️ La diaria de Magma — ${fechaLarga}`
  const info = await t.sendMail({ from:`Somos Magma <${env.MAIL_USER}>`, to:DIARIA_TO, subject, text:texto, html })
  appendFileSync('scripts/.diaria-log.txt', `${hoy.toISOString()} enviada a ${DIARIA_TO} (${info.messageId})\n`)
