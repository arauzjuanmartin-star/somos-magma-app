...
export default function Login() {
  ...
  // A dónde quería ir antes de que le pidiéramos entrar. Los avisos por mail
  // linkean a un trabajo concreto y ahí es donde tiene que caer.
  const volver = typeof router.query.volver === 'string' && router.query.volver.startsWith('/') ? router.query.volver : '/'

  // Si ya está logueado, redirigir a donde quería ir
  useEffect(() => {
    if (status === 'authenticated') router.replace(volver)
  }, [status, router, volver])

  const errorMsg = errorParam === 'AccessDenied'
    ? 'Este mail no está cargado en Magma. Entrá con el mismo mail que tiene Magma en tu ficha, o pedí que lo actualicen.'
    : errorParam
  ...
  return <>
    ...
    <div style={{minHeight:'100vh',background:'#090909',display:'flex',alignItems:'center',justifyContent:'center',padding:20,fontFamily:'system-ui,sans-serif'}}>
      <div style={{width:'100%',maxWidth:380,background:'#0E0E0E',border:'0.5px solid #1F1F1F',borderRadius:14,padding:'40px 32px 32px',boxShadow:'0 8px 32px #000a'}}>
        ...
        <div style={{fontSize:12,color:'#666',marginBottom:30,textAlign:'center'}}>Equipo y freelancers de Magma</div>

        <button onClick={()=>signIn('google',{callbackUrl:volver})} style={{width:'100%',padding:'12px 16px',background:'#fff',color:'#333',border:'none',borderRadius:8,fontSize:14,fontWeight:500,cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',gap:10,boxShadow:'0 2px 6px #0006'}}>
        ...
        <div style={{marginTop:24,fontSize:11,color:'#555',textAlign:'center',lineHeight:1.5}}>
          Entrá con el mail que tiene Magma en tu ficha.<br/>Si no podés entrar, escribile a admin@somosmagma.com.
        </div>
      </div>
    </div>
  </>
