...
export default async function handler(req, res) {
  ...
  try {
    ...
    const cuerpo = [
      ...
      ``,
      `Te paso la factura correspondiente${proyecto?' al proyecto "'+proyecto+'"':''}.`,
      ``,
      `Detalle:`,
      `• Cliente: ${cliente || agencia || '—'}`,
      proyecto ? `• Proyecto: ${proyecto}` : null,
      nroFactura !== 's/n' ? `• N° factura: ${nroFactura}` : null,
      tipoFactura ? `• Tipo: ${tipoFactura}` : null,
      fechaEmision ? `• Fecha emisión: ${fechaEmision}` : null,
      vencimiento ? `• Vencimiento: ${vencimiento}` : null,
      `• Total: $${fmt(total)}${iva > 0 ? ' (incluye IVA)' : ' + IVA'}`,
      ``,
