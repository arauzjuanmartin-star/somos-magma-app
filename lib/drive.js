...
async function buscar(drive, parentId, driveId, nombre) {
  ...
  const exacta = files.find(f => clave(f.name) === k)
...
export async function carpetaProyecto({ nro, fechaEvento, agencia, cliente, proyecto, destinos = ['crudo'], subcarpetas = {}, dryRun = false }) {
  ...
  for (const destino of destinos) {
    ...
    if (esCrudo) {
      ...
      // Entregas: la MISMA estructura que Crudo pero sin el prefijo CR_.
      // Antes iba solo por cliente y por eso creaba "IVECO" en la raíz al lado de
      // "ADN Comunicacion", que ya tenía adentro IVECO, Case y NEW HOLLAND. Con la
      // agencia arriba, el material de una agencia queda junto y se le puede dar
      // acceso a la agencia entera de una.
      niveles = (ag && cli && clave(ag) !== clave(cli)) ? [ag, cli] : [cli || ag || 'SIN_CLIENTE']
    }
    ...
    let renombrada = null
    // En ENTREGAS, la agencia y el cliente tienen su carpeta "Recursos": logo, gráfica
    // y todo lo general que sirve para cualquier trabajo (Juan, 14/9/2026). Se asegura
    // al pasar por cada nivel y el link va a AGENCIAS / CLIENTES ("Drive Recursos").
    const recursos = {}
    const nivelesEntidad = esCrudo ? 0 : niveles.length - 2   // los niveles antes de AÑO
    for (let li = 0; li < niveles.length; li++) {
      const n = niveles[li]
      const esProyecto = li === niveles.length - 1
      if (dryRun) {
      ...
      // Nivel del proyecto: si no existe con este número, buscar la del número
      // ANTERIOR (misma fecha y mismo nombre) y renombrarla. Represupuestar cambia
      // el número pero el rodaje es el mismo — el 14/9 Farmacity #2293 → #2302 armó
      // una "2302_…" vacía al lado de la "2293_…" con 546 videos, y PROYECTOS y la
      // citación quedaron apuntando a la vacía. El ID no cambia al renombrar, así
      // que los links viejos siguen funcionando.
      if (esProyecto && iso) {
        const hay = await buscar(drive, parent, driveId, n)
        if (!hay) {
          const sufijo = n.replace(/^\d+_/, '')
          const r = await drive.files.list({
            q: `'${parent}' in parents and mimeType='application/vnd.google-apps.folder' and trashed=false`,
            driveId, corpora: 'drive', includeItemsFromAllDrives: true, supportsAllDrives: true, pageSize: 200, fields: 'files(id,name)',
          })
          const hermanas = (r.data.files || []).filter(f => /^\d+_/.test(f.name) && f.name.replace(/^\d+_/, '') === sufijo)
          if (hermanas.length === 1) {
            await drive.files.update({ fileId: hermanas[0].id, requestBody: { name: n }, supportsAllDrives: true })
            renombrada = hermanas[0].name
            ruta.push(`${n}  (era ${hermanas[0].name})`); parent = hermanas[0].id
            continue
          }
        }
      }
      ...
      if (!esCrudo && li < nivelesEntidad) {
        try {
          const rc = await asegurarCarpeta(drive, c.id, driveId, 'Recursos')
          recursos[li === 0 && nivelesEntidad === 2 ? 'agencia' : 'cliente'] = { id: rc.id, link: linkDe(rc.id), creada: rc.creada }
        } catch (e) { /* sin Recursos no se frena nada */ }
      }
    }
...
export async function asegurarCarpetasProyecto({ sheets, SHEET_ID, num, destinos = ['crudo', 'entregas'], compartir = false, dryRun = false }) {
  ...
  // Los Recursos de la agencia y del cliente → AGENCIAS / CLIENTES, si la fila existe
  // y la celda está vacía. La columna la crea scripts/drive-recursos.mjs.
  const rec = r.entregas?.recursos || {}
  const k = s => sinTildes(s).toUpperCase().replace(/[^A-Z0-9]/g, '')
  for (const [hoja, nombre, item] of [['AGENCIAS', datos.agencia, rec.agencia], ['CLIENTES', datos.cliente, rec.cliente]]) {
    if (!item?.link || !nombre) continue
    try {
      const rr = await sheets.spreadsheets.values.get({ spreadsheetId: SHEET_ID, range: `${hoja}!A:Z` })
      const rows2 = rr.data.values || [], hh = rows2[0] || [], iR = hh.indexOf('Drive Recursos')
      if (iR < 0) continue
      const fi = rows2.findIndex((x, i) => i > 0 && k(x[0]) === k(nombre))
      if (fi > 0 && !String(rows2[fi][iR] || '').trim()) {
        await sheets.spreadsheets.values.update({ spreadsheetId: SHEET_ID, range: `${hoja}!${colLetra(iR)}${fi + 1}`, valueInputOption: 'USER_ENTERED', requestBody: { values: [[item.link]] } })
      }
    } catch (e) { /* best-effort */ }
  }
