...
// "Film ½", "Film 1", "Film 12hs" es un FILMMAKER: saca fotos y filma. En el catálogo
// hay tres cosas distintas — Foto (fotógrafo), Video (solo cámara) y Film (las dos) —
// así que un presupuesto sin ninguna línea "Foto" igual puede llevar entrega de fotos
// (#2297 Personal / Jamiroquai, 18/9/2026: Film ½ + Edit 60s, y las fotos iban).
export const esPedidoFilm = ped => /^[^\p{L}\p{N}]*film\b/iu.test(String(ped || ''))
// ¿Este proyecto le entrega fotos al cliente?
export const llevaFotos = (pedidos = []) => pedidos.some(p => esPedidoFoto(p) || esPedidoFilm(p))
export const SUB_FOTOS = 'Fotos'

// Dado los pedidos de un proyecto, qué subcarpetas hacen falta.
//
// CRUDO se arma por tipo de material, y SOLO de lo que se vendió: si el trabajo
// no lleva fotos, no se crea una carpeta de Fotos vacía.
//
// ENTREGAS siempre lleva las mismas dos, y son HERMANAS a propósito: en Drive el
// acceso se hereda hacia abajo, así que al cliente se le da "Finales" y nunca ve
// los cortes que rebotamos. Es la división que Popstars ya venía haciendo a mano.
export const SUB_PRE = 'Pre-entregas'
export const SUB_FIN = 'Finales'

export function subcarpetasDe(pedidos = []) {
  const hayFoto  = pedidos.some(esPedidoFoto)
  const hayVideo = pedidos.some(esPedidoVideo)
  const crudo = []
  if (hayFoto)  crudo.push('Fotos')
  if (hayVideo) crudo.push('Videos')
  ...
  return { crudo, entregas: [SUB_PRE, SUB_FIN] }
}
...
//
// Y el trabajo NO termina cuando sale: termina cuando el cliente dice que sí.
// Hasta el 14/9/2026 "Aprobar" cerraba la fila como entregada y el Stand de
// Brasil figuraba listo con el cliente todavía mirándolo. Ahora el OK del PM
// deja la pieza "Con el cliente", y "Terminado" es su OK, no el nuestro.
export const ESTADOS = [
  ...
  'Con el cliente',      // el PM lo aprobó: está en Finales con acceso dado, esperamos SU OK
  'Cambios del cliente', // rebotó el cliente (estas son las 2 rondas del manual)
  'Terminado',           // el cliente dio el OK final — recién acá se cierra
]
// 'Aprobado' y 'Entregado' son del modelo viejo, cuando el OK del PM cerraba todo.
// Se siguen reconociendo como el último paso para no reabrir lo ya cerrado.
const CERRADOS = ['Aprobado', 'Entregado', 'Terminado']
export const estaCerrado = e => CERRADOS.includes(String(e || '').trim())
export const ESTADO_IDX = e => {
  const s = String(e || '').trim()
  if (CERRADOS.includes(s)) return ESTADOS.length - 1
  const i = ESTADOS.indexOf(s); return i === -1 ? 0 : i
}
// El nombre vigente del estado de una fila (los cierres viejos se muestran como Terminado).
export const estadoDe = e => ESTADOS[ESTADO_IDX(e)]
...
// En PROYECTOS, "Somos Magma" en el staff significa que ESA PLATA queda en la
// productora, no que la productora edite. Es una etiqueta de facturación. Si se
// copia al campo Editor, el tablero dice que el trabajo está asignado cuando en
// realidad no lo tiene nadie — que es peor que verlo en blanco.
export const ES_MAGMA = n => /^somos\s*magma$/i.test(String(n || '').trim())
...
// Lo que NO hace falta preguntar al presupuestar porque ya está en el presu.
// Cada pregunta de más es un motivo para no usar el formulario: si el pedido dice
// "Edit 60s", la duración ya está dicha.
export const duracionDePedido = ped => {
  const p = String(ped || '')
  if (/15\s*-?\s*30/.test(p)) return '15-30 s'
  if (/60\s*s\s*\+/i.test(p)) return 'Más de 60 s'
  if (/45\s*s/i.test(p)) return '45 s'
  if (/60\s*s/i.test(p)) return '60 s'
  return ''
}
// Si el presu tiene jornadas de cámara o de foto, el material lo filmamos nosotros.
// Un presu de sola edición es material del cliente, que es otro trabajo.
export const materialDePedidos = peds =>
  (peds || []).some(p => !esPedidoEdicion(p) && (esPedidoVideo(p) || esPedidoFoto(p)))
    ? 'Lo filmamos nosotros' : 'Lo pone el cliente'
...
export function semaforo(fila, hoy = hoyCero()) {
  ...
  if (estaCerrado(estado)) {
    return { nivel: 'listo', orden: 100, txt: 'Terminado' }
  }
  ...
  // Ya se le entregó: el plazo se cumplió (o no) el día de la entrega, no hoy.
  // Si acá contara contra la fecha comprometida, cada pieza esperando el OK del
  // cliente se pondría roja a los pocos días y el rojo dejaría de ser culpa
  // nuestra. Cuánto tarda el cliente se ve en el chip "Con el cliente" y en el
  // panel de la fila, que a los 5 días pide un llamado.
  if (esperaAlCliente(estado)) {
    const entregado = parseFechaAR(fila['Fecha entrega'])
    const dias = entregado ? diasEntre(entregado, hoy) : null
    const txt = dias === null ? 'con el cliente' : dias === 0 ? 'con el cliente desde hoy' : `con el cliente hace ${dias} ${dias === 1 ? 'día' : 'días'}`
    return { nivel: 'verde', orden: 60 - Math.min(dias || 0, 30), txt }
  }

  // La fecha la tiene que poner el PM. El plazo del manual sirve de sugerencia
  // al cargar, pero no como compromiso: el orden real lo da la prioridad del
  // cliente, y algunos trabajos se cotizan por emergencia con otro plazo.
  const puesta = parseFechaAR(fila['Fecha compromiso'])
  const compromiso = puesta || fechaSugerida(fila['Fecha Evento'], fila.Entregable)
  ...
  if (!compromiso) {
    return { nivel: 'naranja', orden: -40, txt: 'falta la fecha' }
  }
  // Sin fecha puesta a mano, lo que se muestra es una estimación, no una promesa.
  if (!puesta) {
    const faltan0 = diasEntre(hoy, compromiso)
    return {
      nivel: faltan0 < 0 ? 'rojo' : 'naranja',
      orden: faltan0 - 20,
      txt: faltan0 < 0 ? 'falta la fecha · el plazo ya pasó' : 'falta la fecha',
    }
  }
  ...
  // Urgente NO cambia el nivel: lo adelanta en el orden (abajo) y ya se ve con su
  // etiqueta roja en la fila. Antes subía un escalón y eso rompía los chips: una
  // tarea urgente que vencía hoy aparecía bajo "Atrasado", así que buscándola en
  // "Vence hoy" no estaba. El chip tiene que decir la verdad del plazo.
...
export const COLOR_SEM = {
  ...
  listo:    { fg: '#8C8880', bg: '#F2F0EC', label: 'Terminado' },
}
...
export const HEADERS_EDICION = [
  ...
  // Las columnas nuevas van SIEMPRE AL FINAL: el setup las agrega ahí, y meter
  // una en el medio del array corre todo lo que se escribe por posición.
  'Interno',            // el slot lo cobra Magma — igual alguien lo tiene que hacer
]
