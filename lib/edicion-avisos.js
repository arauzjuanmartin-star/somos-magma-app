...
const AVISOS = {
  ...
  'Cambios internos':    { a: 'editor', asunto: f => `Cambios: ${titulo(f)}`,               cuerpo: f => `Te pidieron correcciones antes de mandarlo al cliente.${loQuePidieron(f)}` },
  'Cambios del cliente': { a: 'editor', asunto: f => `Cambios del cliente: ${titulo(f)}`,   cuerpo: f => `El cliente pidió correcciones.${loQuePidieron(f)}` },
}
...
export function armarAviso({ fila, estadoNuevo, rrhh = [], mailQuienCambio = '' }) {
  ...
  const cuerpo = [
    ...
    `Abrirlo en el tablero (para aprobar o pedir cambios):\n${enLaApp}`,
    '',
    '—',
    titulo(fila),
