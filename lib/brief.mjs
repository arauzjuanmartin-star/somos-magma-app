...
export function calcularBrief({ PRE, PRO, FAC, RH }, ahora = new Date()) {
  ...
  // La col G (Fecha Evento) quedó vacía en todas las facturas cargadas desde la app hasta el 24/09/2026: "atrasadas +30d"
  // daba $0 con $28M de eventos viejos sin cobrar. Si falta, se toma la fecha del proyecto (o del presupuesto) por N°.
  const eventoPorNro = new Map()
  PRE.slice(1).forEach(p => { const n = txt(p[0]), d = fecha(p[1]); if (n && d && !eventoPorNro.has(n)) eventoPorNro.set(n, d) })
  PRO.slice(1).forEach(p => { const n = txt(p[2]), d = fecha(p[3]); if (n && d) eventoPorNro.set(n, d) })
  const facturas = FAC.slice(1).filter(f => txt(f[1]) || txt(f[8])).map(f => ({
    nro: txt(f[1]), cobrado: esTrue(f[4]), fEvento: fecha(f[6]) || eventoPorNro.get(txt(f[1])) || null, agencia: txt(f[7]), cliente: txt(f[8]), proyecto: txt(f[9]),
